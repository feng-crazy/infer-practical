...
class TrtNetworkHelper():
    ...
    def addLayerNorm(self, x, gamma, beta, layer_name=None, precision=None):
        ...
        gamma = self.network.add_constant(gamma.shape, trt.Weights(gamma)).get_output(0)
        beta = self.network.add_constant(beta.shape, trt.Weights(beta)).get_output(0)
    ...
    def addLinear(self, x, weight, bias, layer_name=None, precision=None):
        ...
        trt_layer = self.network.add_fully_connected(x, weight.size, weight, bias)
    ...
    def addAdd(self, a, b, layer_name=None, precision=None):
        ...
        if layer_name is None:
            layer_name = "torch.add"
        else:
            layer_name = "torch.add." + layer_name
    ...
    def addScale(
        ...
        trt_layer = self.network.add_constant((1,), scale_weights)
        scale_tensor = trt_layer.get_output(0)
        
        trt_layer = self.network.add_elementwise(x, scale_tensor, trt.ElementWiseOperation.PROD)
